            ...
            if (SysProperties.CONSOLE_STREAM && server.getAllowChunked()) {
                ...
                session.put("chunks", new Iterator<String>() {
                    ...
                    public String next() {
                        ...
                        if (s.equalsIgnoreCase("SELECT * FROM TEST")) {
                            ...
                            Future<?> future = executor.submit(new Runnable() {
                                @Override
                                public void run() {
                                    query(conn, str, i - 1, list.size() - 2, b);
                                }
                            });
                            ...
                            try {
                                ...
                                future.cancel(true);
                                //System.out.println("Create index and retry");
                                //s = "CREATE INDEX Amanda ON TEST(NAME)";
                ...
                //-----------------------------------------------------
                if(timedOut) {
                    System.out.println("Time Out="+timedOut);
                    timedOut=false;
                    session.remove("chunks");
                    String sql1 = "CREATE INDEX Amanda ON TEST(NAME);\r\nSELECT * FROM TEST;";
                    ScriptReader r1 = new ScriptReader(new StringReader(sql1));
                    final ArrayList<String> list1 = New.arrayList();
                    while (true) {
                        String s = r1.readStatement();
                        if (s == null) {
                            break;
                        }
                        list1.add(s);
                    }
                    final Connection conn1 = session.getConnection();
                    if (SysProperties.CONSOLE_STREAM && server.getAllowChunked()) {
                        String page1 = new String(server.getFile("result.jsp"), Constants.UTF8);
                        int idx1 = page1.indexOf("${result}");
                        // the first element of the list is the header, the last the
                        // footer
                        list1.add(0, page1.substring(0, idx1));
                        list1.add(page1.substring(idx1 + "${result}".length()));
                        session.put("chunks", new Iterator<String>() {
                            private int j;
                            ...
                            public String next() {
                                ...
                                query(conn, s, j - 1, list1.size() - 2, b);
                                return b.toString();
                            }
